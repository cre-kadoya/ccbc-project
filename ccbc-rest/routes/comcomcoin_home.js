...
router.post('/findHome', (req, res) => {
  ...
  resdatas.newArticleList = [
    { t_kiji_pk: 1, title: "マラソン大会へのお誘い", hashtagStr: "#スポーツ　#マラソン　", file_path: "test001.png", goodCnt: 12 },
...
function getHomeOshirase(db, req) {
  return new Promise((resolve, reject) => {
    // 最新の3件を取得
    var sql =
      "select top 3 renban, notice_dt, title" +
      " from t_oshirase" +
      " where delete_flg = '0'" +
...
function getHomeKiji(db, req, isNew) {
  return new Promise((resolve, reject) => {
    ...
    if (isNew) {
      ...
      sql += " order by coalesce(goo.cnt, 0), kij.post_dt desc, kij.post_tm desc"
    }
    sql += " limit 3"
  ...
  return new Promise((resolve, reject) => {
    ...
    switch (mode) {
      ...
      case "popular":
        sql +=
          " and kij.post_dt >= current_timestamp + '-1 months'" +
          " order by coalesce(goo.cnt, 0), kij.post_dt desc, kij.post_tm desc"
        break
      case "favorite":
        sql +=
